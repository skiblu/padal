/*
  ...
  ES module retro cassette player
  - Modes: single | playlist | radio (cannot switch)
  - Playlist available for playlist & radio
  - Radio disables seeking, starts at radioStart
  - All icons use SVG from assets/bootstrap-icons/
  - Playlist toggle uses list.svg
*/
...
class MusicPlayer {
      ...
      <div class="cassette-player">
        <div class="cassette-shell">
          ...
          <div class="mp-playlist-container" style="display:none; margin-top:10px;">
            <button class="mp-playlist-toggle" title="Playlist">
  ...
  _injectStyles(){
    ...
    style.textContent=`
      /* CSS same as previous (omitted here for brevity) */
    `;
  ...
  _togglePlaylist(show){
    ...
    if(this.playlistToggle) this.playlistToggle.title=show?'Hide Playlist':'Playlist';
  ...
  setMode(m){ this.mode=m; this.isLive=this.mode==='radio'; if(this.mode==='playlist'||this.mode==='radio'){this.playlistContainer.style.display=''; this._buildPlaylistView(); this._togglePlaylist(false);} else this.playlistContainer.style.display='none'; this.seekRange.disabled=this.mode==='radio'; this.seekRange.style.opacity=this.mode==='radio'?0.5:''; if(this.isLive && this.playlist.length){ this._loadTrack(this.currentIndex,false); if(this.options.radioStart) try{ this.audio.currentTime=this.options.radioStart;}catch(e){} } }
  ...
  _attachEvents(){
    ...
    this.seekRange.addEventListener('input',()=>{ if(this.mode!=='radio') this.audio.currentTime=this.seekRange.value; });
...
// Helper initializer
function initMusicPlayer(container, options){ return new MusicPlayer(container, options); }
